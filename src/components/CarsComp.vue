  ...
  <!-- <div class="kocsik">
    <div class="asd">
      <div v-for="car in this.$store.getters.cars" :key="car.id" class="kocsi">
        <p>{{ car.name }}</p>
        <p>{{ car.description }}</p>
        <img :src="require('../views/kepek/' + car.url + '.png')" />
        <router-link tag="button" :to="{ name: 'configurator', params: { id: `${car.url}` } }">Vedd meg</router-link>
      </div>
    </div>
  </div> -->
  <div class="main">
    <b-card class="cars w-75">
      <b-row v-for="car in this.$store.getters.cars" :key="car.id" class="car">
        <b-col class="md">
          <img :src="car.pictureUrl" class="img pl-5 ml-5" />
        </b-col>
        <b-col class="md">
          ...
          <router-link
            tag="button"
            :to="{ name: 'configurator', params: { id: `${car.url}`, pictureUrl: `${car.pictureUrl}` } }"
            class="btn btn-danger"
            >Konfigurálja meg</router-link
          >
        </b-col>
      </b-row>
    </b-card>
